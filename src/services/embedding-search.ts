...
export async function searchEmbeddings(
  ...
  try {
    ...
    console.log(
      `✓ Retrieved ${results.length} embeddings from ${store.isInMemory ? "memory" : "PostgreSQL"}`,
    );
    return results;
  } catch (error) {
...
export async function listStoredEmbeddings(
  ...
  try {
    ...
    console.log(
      `Storage Type: ${store.isInMemory ? "Memory" : "PostgreSQL with pgvector extension"}`,
    );
    console.log(
      `Database: ${process.env["PG_DATABASE"] || "embeddings"} @ ${process.env["PG_HOST"] || "localhost"}:${process.env["PG_PORT"] || 5432}`,
    );
  } catch (error) {
...
export async function clearAllEmbeddings(store: EmbeddingStore): Promise<void> {
  try {
    console.log(
      `🗑️  Clearing all embeddings from ${store.isInMemory ? "memory" : "ChromaDB"}...`,
    );
    await store.clear();
